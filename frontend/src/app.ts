...
export class InterviewApp extends LitElement {
  // Render into light DOM so Tailwind classes apply
      ...
      <main class="min-h-screen bg-slate-950 text-slate-100 px-4 py-8">
        <div class="mx-auto max-w-4xl rounded-3xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl shadow-slate-950/30">
          <header class="mb-8">
            ...
            <p class="mt-2 text-slate-400">Upload your job description and resume, then join a live mock interview.</p>
          </header>
    ...
    return html`
      <section class="space-y-6">
        <div class="grid gap-6 sm:grid-cols-2">
          <div class="space-y-3">
            <label class="text-sm font-medium text-slate-200">Job description</label>
            <textarea
              class="w-full min-h-[160px] rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 shadow-inner focus:border-sky-400 focus:outline-none"
              .value=${this.jobDescription}
              @input=${(e: Event) => { this.jobDescription = (e.target as HTMLTextAreaElement).value; }}
              placeholder="Paste job requirements here"
            ></textarea>
          </div>
          <div class="space-y-3">
            <label class="text-sm font-medium text-slate-200">Resume</label>
            <textarea
              class="w-full min-h-[160px] rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 shadow-inner focus:border-sky-400 focus:outline-none"
              .value=${this.resumeText}
              @input=${(e: Event) => { this.resumeText = (e.target as HTMLTextAreaElement).value; }}
        ...
        <div class="grid gap-6 md:grid-cols-2">
          ...
          <div class="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
            ...
            <ul class="mt-3 space-y-2 text-slate-400">
              <li>• Speak naturally — the agent will wait for you to finish</li>
  ...
  private async startSession() {
    ...
    if (!this.jobDescription.trim() || !this.resumeText.trim()) {
  ...
  private async connectRoom(token: string) {
    ...
    this.room.on(RoomEvent.ActiveSpeakersChanged, (speakers) => {
      ...
      this.status = agentSpeaking ? 'agent-speaking' : 'listening';
    });

    // Receive summary and status via LiveKit text streams
    ...
    this.room.registerTextStreamHandler('interview.status', async (reader) => {
      for await (const chunk of reader) {
        if (chunk.trim() === 'completed') {
          this.screen = 'post-session';
        }
      }
    });
